...
Jenkins.prototype.validateJenkinsfile = function(jenkinsfile, callback) {
  ...
  var expected = 'Jenkinsfile successfully validated.';

  return this._post(
    req,
    function validateJenkinsfileMiddleware(request, next) {
      var received = request.res.body.toString().trim();
      request.res.body = received;
