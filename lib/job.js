...
function makeMultipart(parts) {
  ...
  Object.keys(parts).forEach(function(key) {
    if (typeof parts[key] === 'string') {
      reslist.push({ name: key, value: parts[key] });
    } else {
      if (!Buffer.isBuffer(parts[key])) {
	console.log(parts[key]);
	throw new Error('invalid parameter, must be string or buffer');
      }
      reslist.push({ name: key, file: key });
      multipart[key] = { 'Content-Type': 'application/octet-stream',
			 body: parts[key] };
    }
  });

  multipart.json = JSON.stringify({ parameter: reslist });
...
Job.prototype.build = function(opts, callback) {
  ...
  try {
	...
	if (typeof params[key] !== 'string') {
      ...
      if (multipart) {
	req.multipart = makeMultipart(params);
