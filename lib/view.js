...
View.prototype.configList = function(opts, callback) {
  ...
  this.jenkins._log(['debug', 'view', 'configList'], opts);

  var req = {
    path: '/view/{name}/configSubmit',
    params: { name: opts.name },
    type: 'form',
    name: 'view.configList',
  };
  ...
  return this.jenkins._post(
    req,
    middleware.require302('failed to config: ' + opts.name),
    middleware.empty,
    callback
  );
};
