...
View.prototype.create = function(opts, callback) {
  ...
  var mode = {
    ...
    nested: 'hudson.plugins.nested_view.NestedView',
    dashboard: 'hudson.plugins.view.dashboard.Dashboard',
    buildpipeline: 'au.com.centrumsystems.hudson.plugin.buildpipeline.BuildPipelineView',
    deliverypipeline: 'se.diabol.jenkins.pipeline.DeliveryPipelineView',
  };

  try {
    var path = utils.parsePath(opts.name);

    if (!path.name) throw new Error('name required');
    if (!opts.type) throw new Error('type required');
    if (!mode[opts.type]) throw new Error('type unknown: ' + opts.type);

    req.path = '{folder}/createView';
    req.type = 'form';
    req.params = { folder: path.folder };
    req.body = {
      name: path.name,
      mode: mode[opts.type],
      json: JSON.stringify({
...
View.prototype.remove = function(opts, callback) {
  ...
  var req = {
    path: '/view/{name}/removeJobFromView',
    query: { name: opts.job },
    type: 'form',
    name: 'view.remove',
    body: {},
